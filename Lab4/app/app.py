...
def login():
    if request.method == 'POST':
        ...
        with mysql.connection.cursor(named_tuple=True) as cursor:
            ...
            if db_user:
                login_user(User(user_id=db_user.id,
                           login=db_user.login), remember=remember_me)
...
def create():
    ...
    with mysql.connection.cursor(named_tuple=True) as cursor:
        try:
            ...
            flash('Введены некорретные данные. Ошибка сохранения', 'danger')
            return render_template('/userus/new.html', user=params, roles=load_roles())
